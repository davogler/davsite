...
# In urls.py for example, ('^blog/page/(?P<page>\d)/$', get_archive_index),
	url(r'^$', 'coltrane.views.get_archive_index_first', ),
	url(r'^page/(?P<page>\d)/$', 'coltrane.views.get_archive_index', ),
	#(r'^$', 'django.views.generic.date_based.archive_index', entry_info_dict,  'coltrane_entry_archive_index'),
	#(r'^(?P<year>\d{4})/$', YearArchiveView.as_view(), entry_info_dict, 'coltrane_entry_archive_year'),
	url(r'^(?P<year>\d{4})/$', YearArchiveView.as_view(**entry_info_dict), name= 'coltrane_entry_archive_year'),
